...
But it often happens that we only think about them when we are already knee-deep in problems, maybe even constrained by an impending deadline. Spending time to improve the code in that context could mean unexpected delays. So we are faced with a dilemma: do we continue in the current state or take a step back and risk an agreed delivery?

## Why don't we do the right thing?
It seems reasonable to conclude that unless we apply these practices from the very beginning of a project, we won't reap the benefits later when the pressure to deliver increases.
...
So when a colleague recently asked me how I make gardening work for me when others seem to struggle, I thought I could share some additional ideas. I admit, however, that this was a difficult exercise because many of the micro-decisions I make are driven by intuition. But there are certain patterns that I could identify.
...
So if you want to build the habit of gardening, doing something always is maybe more important than the impact of an improvement itself.
...
Complex changes are trickier because they usually have a larger scope. When there's too much unmanaged technical debt or an overall design that prevents making changes easily, it's tempting to stop planned work and spend time on solving that only before continuing. And while there are cases where this is unfortunately the way to go, in my experience it's rarely a good idea, let alone feasible. Such big changes are _not_ gardening.
