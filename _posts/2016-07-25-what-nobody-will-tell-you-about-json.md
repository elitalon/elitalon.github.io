...
`NSJSONSerialization` was introduced in 2011 with iOS 5 and has been a _de facto_ class to serialise and deserialise JSON objects. The public interface is minimal, with two main methods:
...
4. Using the native `Dictionary` type, but wrapped into an object that handles all the error conditions
5. Keeping error handling locally by using a `do` statement. From a higher level perspective we only wanted to know if the object could be created or not. Hence, the failable initialiser
